...
30 USF undergraduates enrolled in introductory psychology courses participated 
in the study. Students were given class credit for their participation.
...
REdat.totalmeans.table <- REdat.totalmeans %>% 
  select(Condition, RunPosition, RT_Mu, sd, se, N)
datatable(REdat.totalmeans.table)
```
...
REdat.IDanalysismeans.table <- REdat.IDanalysismeans %>% 
  select(ID, Condition, RunPosition, RT_Mu, RT_Sigma, RT_n)
datatable(REdat.IDanalysismeans.table)
```
...
REdat.fullanalysismeans.table <- REdat.fullanalysismeans %>% 
  select(Condition, RunPosition, RT_Mu, sd, N) 
datatable(REdat.fullanalysismeans.table)
```
...
datatable(REdat.IDanalysismeans.congruency)
```
...
datatable(REdat.IDanalysismeans.congruency.means)
```
...
datatable(REdat.IDanalysismeans.CE_difference.means)
```
...
#Cohen's d for first two planned comparisons
pwc1_cohens_d = 33.915828/45.2227
pwc2_cohens_d = 24.491053/40.45816

cohens_d = rbind(pwc1_cohens_d, pwc2_cohens_d)
...
pwc.1_and_2 <- REdat.IDanalysismeans %>%
  ...
  mutate(`Cohen's D` = cohens_d)


pwc.1_and_2 %>%
  kbl(caption = "Pairwise comparison: Local Incongruent at P2 (and 6) vs. Local congruent at P2 (and 6))") %>%
...
pwc.3 <- REdat.IDanalysismeans.congruency %>%
  ...
  pairwise_t_test(
    ...
    p.adjust.method = "bonferroni") %>% 
  mutate(`Cohen's D` = 9.424776/11.62251)

pwc.3 %>% 
  kbl(caption = "Pairwise comparison: Difference at position 2 vs. differeance at position 6") %>%
  kable_classic(full_width = F, html_font = "Cambria")
```
...
Hoffman, J. E. (1980). Interaction between global and local levels of a form. 
  Journal of Experimental Psychology: Human Perception and Performance, 6(2), 
  222.

Kimchi, R. (1992). Primacy of wholistic processing and global/local paradigm: a 
  critical review. Psychological bulletin, 112(1), 24.
...
Miller, J. (1981). Global precedence in attention and decision. Journal of 
  Experimental Psychology: Human Perception and Performance, 7(6), 1161.
...
Navon, D., & Norman, J. (1983). Does global precedence reality depend on visual 
  angle?. Journal of Experimental Psychology: Human Perception and Performance, 
  9(6), 955.

Ward, L. M. (1982). Determinants of attention to local and global features of 
