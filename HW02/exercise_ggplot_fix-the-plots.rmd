...
geom_jitter(width = 0.8, height = 0.8, size=.9, color = "red")
...
ggplot(data=gapminder_noOceania) +
  aes(gdpPercap, lifeExp, fill=country, color=continent,size=pop) +
  geom_point(alpha = 0.7,shape=21, show.legend = FALSE) +
  scale_size_area()+
...
ggplot(caribou_path) +
  aes(x, y, label=caribou_path$time) +  
  geom_text(vjust = -1) +
  ylim(.25,1.2)+
...
ggplot(americas) +
  ...
  geom_density_ridges(scale = 10, size = 0.25, rel_min_height = 0.03,bandwidth = 1)+
...
ggplot(mtcars2) +
  aes(cyl,y = (..count..)/sum(..count..),fill=transmission) +
  facet_wrap(~ transmission) +
  ylab("Proportions") +
  scale_fill_manual(values=c("#899999", "#E69F00"),name = "Transmission") +
...
ggplot(hair_eye, aes(Hair, Eye)) +
  geom_tile(aes(x = Hair,
                        y = Eye,
                        fill = Eye, alpha = Hair, color = Hair))
```
